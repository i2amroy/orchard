...
from ..file import LogFile, LinkFile, ConfigFile
...
def branching2(config_file, link_file, workspace_path):
    ...
    if not work_path.is_file():
        ...
        branch_config_struct(config_file, "1")
    ...
    for branchnum, branchconfig in LogFile.configs.items():
        ...
        if perfect_branch:
            branch_config_struct(config_file, branchnum)
            return config_file
        ...
        for module in branch_matching:
            for path in module.get_dynamic_paths():
        ...
        branch_config_struct(config_file, new_branchnum)
        return config_file


def branch_config_struct(config_file, branchnum):
    # TODO
    pass
